...
exports.update = withDatabase(function (model) {
  ...
  model.save(function (err, model) {
    ...
    self._logger.info('%s->update: %s ms', self.name, duration);
...
exports.getStream = withDatabase(function (searchBy, fields) {
  ...
  this._logger.info('%s->getStream: %s', name, JSON.stringify(searchBy));
  ...
  stream.on('err', function (err) {
    computeDuration();
    self._logger.error('%s->getStream error: %s', name, String(err));
  });
  ...
  function computeDuration() {
    ...
    self._logger.info('%s->get query time for %s: %s ms',
...
exports.getOne = withDatabase(function (searchBy) {
  ...
  var self = this;
  this._logger.info('%s->getOne: %s', name, JSON.stringify(searchBy));
  ...
  .then(null, function (err) {
    computeDuration();
    self._logger.error('%s->getOne error: %s', name, String(err));
    throw err;
  });
  ...
  function computeDuration() {
    var endTime = new Date();
    var duration = endTime.getDate() - startTime.getDate();
    self._logger.info('%s->getOne query time for %s: %s ms',
                    name, JSON.stringify(searchBy), duration);
  }
});
...
exports.findOneAndUpdate = withDatabase(function (searchBy, update, options) {
  ...
  var self = this;
  this._logger.info('%s->findOneAndUpdate: %s', name, JSON.stringify(searchBy));
  ...
  .then(null, function (err) {
    computeDuration();
    self._logger.error('%s->findOneAndUpdate error: %s', name, String(err));
    throw err;
  });
  ...
  function computeDuration() {
    var endTime = new Date();
    var duration = endTime.getDate() - startTime.getDate();
    self._logger.info('%s->findOneAndUpdate query time for %s: %s ms',
                    name, JSON.stringify(searchBy), duration);
  }
});
